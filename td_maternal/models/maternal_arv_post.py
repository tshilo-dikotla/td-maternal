...
from ..maternal_choices import (
    REASON_FOR_HAART, ARV_DRUG_LIST, DOSE_STATUS, ARV_MODIFICATION_REASON)
from .model_mixins import CrfModelMixin


class MaternalArvPost (CrfModelMixin):

    """ A model completed by the user on the mother's ARVs administered
    post-partum.
    """
    ...
    on_arv_since = models.CharField(
        max_length=25,
        choices=YES_NO,
        verbose_name=("Was the mother supposed to be on triple ARVs any time "
    ...
    arv_status = models.CharField(
        verbose_name=("What is the status of the participant's antiretroviral"
                      " treatment / prophylaxis at this visit or "
...
class MaternalArvPostMed(BaseUuidModel):
    ...
    def natural_key(self):
        return ((
            self.arv_code, self.modification_date) +
            self.maternal_arv_post.natural_key())
